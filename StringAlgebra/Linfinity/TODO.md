...
2. `StringAlgebra/Linfinity/*.lean` currently carries 3 explicit theorem-level `sorry` markers for active proof gaps (`LInfinityAlgebra.lean`: 1, `Transfer.lean`: 2); there is no `sorry` in `def`/`structure`/`abbrev` bodies.
...
5. Recent hardening completed:
   ...
   - `Transfer.lean`: removed proof-carrying fields `TransferResult.inclusion_isQuasiIso`, `MinimalModelResult.quasiIso_property`, and `FormalityResult.quasiIso_property`; quasi-isomorphism claims are now theorem-level obligations (with remaining gaps at `minimalModelMorphism_isQuasiIso`, `formalityMorphism_isQuasiIso`).
   ...
   - `Formality.lean`: removed tautological physical-interpretation wrappers (`canonicalCommutator_firstOrder`, `poissonSigmaModel_witness`) in favor of direct use of witness fields.
   ...
   - `Formality.lean`: discharged `kontsevichFormality_is_quasi_iso` from `HKRMap.induces_iso` using the explicit component and HKR normalization specs (`component_spec`, `linear_hkr_spec`).
   - `Formality.lean`: discharged `deformationQuantization` by constructing an explicit formal MC element from the HKR chain-map witness at order `ℏ¹`.
   - `Formality.lean`: discharged `starProductClassification` from the current gauge-transformation interface (whose `firstOrder` field is exactly first-order Poisson-bracket equality).
   ...
   - `Formality.lean`: aligned theorem naming/documentation with witness-level semantics (`hkr_chain_equation`) and removed historical compatibility-alias wrappers for naming drift.
   ...
   - `SymmetricCoalgebra.lean`: removed `CoalgebraAxioms` contract bundling; coalgebra properties are now exposed as direct theorem statements (`coproduct_coassociative`, `counit_axiom`, `coproduct_graded_cocommutative`).
   ...
   - `MaurerCartan.lean`: strengthened `MCTheory` by linking gauge action to gauge-equivalence data (`gaugeAction_zero`, `gaugeAction_sound`) and used these fields directly in the canonical setoid/equivalence wiring.
   - `MaurerCartan.lean`: strengthened `MCTheory` with MC-preservation under gauge action (`gaugeAction_preserves_curvature_zero`) and used this field directly in canonical gauge action on MC elements (`MCElement.gaugeAct`).
   - `MaurerCartan.lean`: kept the moduli quotient interface minimal at the canonical equivalence theorem (`MCElement.toModuli_eq_iff_gaugeEquivalent`) and removed redundant directional forwarding wrappers.
   - `MaurerCartan.lean`: aligned gauge/twisting/tangent-obstruction documentation with witness-level semantics and removed historical compatibility-alias wrappers plus tautological MC-witness forwarding theorem shell.
   - `Coderivations.lean`: strengthened `ReducedCoderivation` with explicit `component_spec` consistency (on word-length-`n` inputs, `componentMap n` agrees with `map`) and explicit unary-output shape (`component_wordLength_one`), exposed vanishing equivalence (`vanishesOnWordLength_iff_componentMap`) to keep map-vs-component formulations synchronized, added an explicit square-zero decomposition interface by word length (`squareVanishesOnWordLength`, `isSquareZero_iff_squareVanishesOnAllWordLengths`), added bracket-level characterizations for `isDGLA`/`isLieAlgebra`/`isLie2Algebra` with witness-level naming while removing historical compatibility aliases, and removed redundant component/bracket forwarding theorem shells.
...
isFormal_unpacked_with_linear_bijectivity
<-> unpacked_with_linear_bijectivity_iff_exists_formalityLinear_isBijective

isFormal_unpacked
<-> unpacked_iff_exists_formalityLinear_isBijective
