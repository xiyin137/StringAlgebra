...
5. `SymmetricCoalgebra.lean`: medium risk. Uses representational `Bool`-style zero tracking; `SymPower` and `SymCoalg` now enforce stored total-degree/degree consistency and zero-marker degree-vanishing constraints, but non-quotiented semantics remain limited.
